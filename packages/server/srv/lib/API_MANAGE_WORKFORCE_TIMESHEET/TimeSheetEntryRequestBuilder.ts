 ...
 */
export class TimeSheetEntryRequestBuilder<
...
> extends RequestBuilder<TimeSheetEntry<T>, T> {
   ...
   */
  create(
    entity: TimeSheetEntry<T>
  ): CreateRequestBuilder<TimeSheetEntry<T>, T> {
    return new CreateRequestBuilder<TimeSheetEntry<T>, T>(
      this.entityApi,
      entity
    );
  }
